...
def get_mean_residue_ellipticity(
    ...
    """
    Calculates the conversion factor from mdeg to Mean Residue Ellipticity (MRE; degxcm²xdmol⁻¹)
    ...
    if not all([protein_concentration_uM, path_length_mm, num_residues]):
        raise ValueError(
            "Missing one or more parameters for MRE calculation: concentration, path length, num residues"
        )
    ...
    if denominator == 0:
        raise ValueError(
            "MRE calculation denominator is zero. Check concentration, path length, and num residues."
        )
    return 1.0 / denominator


def interpolate_and_smooth_data(
    x_values, y_values, new_x_grid=None, window_length=7, polyorder=2
):
    """
    ...
    try:
        interp_func = interp1d(
            x_sorted, y_sorted, kind="cubic", bounds_error=False, fill_value=np.nan
        )
    ...
    if len(valid_y) > polyorder and len(valid_y) > window_length:
        if window_length % 2 == 0:
            window_length += 1
        window_length = min(
            window_length, len(valid_y) - (1 if len(valid_y) % 2 == 0 else 0)
        )
        if window_length > polyorder:
            try:
                ...
                print(
                    f"Warning: Savgol filter failed: {e}. Returning interpolated data."
                )
        else:
            print(
                f"Warning: window_length ({window_length}) must be greater than polyorder ({polyorder}). Skipping smoothing."
            )
    else:
        print(
            f"Warning: Not enough data points ({len(valid_y)}) for smoothing. Skipping smoothing."
        )
...
def plot_cd_spectra(
    ...
    if overlay_df is not None:
        ...
        new_wave_overlay, smooth_overlay = interpolate_and_smooth_data(
            overlay_wave,
            overlay_vals,
            new_x_grid=new_wave,
    ...
    sns.set_style("ticks")
    sns.lineplot(
        data=smooth_df.dropna(),
        x="Wavelength_nm",
        y="Smoothed_MRE_deg_cm2_dmol",
        linewidth=1.5,
        label=f"{orientation_code} Pre-melt",
        color="black",
    )
    if overlay_smooth_df is not None:
        sns.lineplot(
            data=overlay_smooth_df.dropna(),
            x="Wavelength_nm",
            y="Smoothed_MRE_deg_cm2_dmol",
            color=red_color,
            linestyle="--",
            linewidth=1.5,
            label=f"{orientation_code} Post-melt",
        )

    full_title = f"{title}\n{orientation_code}" + (
        f" Pre vs Post-Melt" if overlay_df is not None else "Pre-Melt"
    )
    ...
    filename_base = (
        f"cd_spectrum_{well_name}_{orientation_code.replace('.', '_')}_pre_melt"
    )
    if overlay_df is not None:
        filename_base += (
            f"_vs_{well_name}_{orientation_code.replace('.', '_')}_post_melt"
        )
    for ext in ["png", "svg"]:
        plt.savefig(
...
def plot_melting_curve(
    ...
    num_residues,
    wavelength_nm=222,
    reverse_melt_df=None,
    reverse_label="Cooling",
    title="Thermal Melting Curve",
    save_dir=".",
    y_limits_mre=None,
    x_limits=None,
):
    ...
    for df in [melt_data_df] + (
        [reverse_melt_df] if reverse_melt_df is not None else []
    ):
        if not all(col in df.columns for col in required_cols):
            raise ValueError(f"DataFrame missing required columns: {required_cols}")
    ...
    melt_data = melt_data.sort_values(by="Temperature_C").dropna(
        subset=["MRE_deg_cm2_dmol"]
    )
    ...
    if reverse_melt_df is not None:
        ...
        reverse_data = reverse_data.sort_values(by="Temperature_C").dropna(
            subset=["MRE_deg_cm2_dmol"]
        )
    ...
    sns.lineplot(
        ...
        y="MRE_deg_cm2_dmol",
        linewidth=1.5,
        label=f"{orientation_code} (Heating)",
        color=heat_color,
    )
    if reverse_data is not None:
        sns.lineplot(
            ...
            linewidth=1.5,
            linestyle="--",
            label=f"{orientation_code} ({reverse_label})",
            color=cool_color,
        )
    ...
    plt.legend(custom_lines, ["Heating", "Cooling"], loc="best", fontsize=10)
    plt.tight_layout()
    filename_base = f"melting_curve_{well_name.replace(' ', '_')}_{orientation_code.replace('.', '_')}_{wavelength_nm}nm"
    if reverse_data is not None:
...
def subtract_blank_from_sample(parsed_data, key, blank_df):
    ...
    if key in parsed_data and "data" in parsed_data[key]:
        ...
        if (
            sample_df.columns[0] == blank_df.columns[0]
            and len(sample_df) == len(blank_df)
            and np.allclose(sample_df.iloc[:, 0], blank_df.iloc[:, 0])
        ):
            y_col = sample_df.columns[1]
            subtracted = sample_df.copy()
            subtracted[y_col] = sample_df[y_col] - blank_df[y_col]
            parsed_data[f"{key}_Subtracted"] = {
                "meta": parsed_data[key]["meta"],
                "data": subtracted,
            }
        else:
            print(
                f"Warning: X-axis mismatch or unequal lengths for {key}. Skipping blank subtraction for this file."
            )
    else:
        print(f"Warning: {key} not found in parsed data.")


def process_sample(sample, params_df, parsed_data, params, blank_df, orientation_code):
    """
...
if __name__ == "__main__":
    ...
    data_dir = (
        "/home/tadas/code/deltaproteinsBristol/experimental_results/bristol_cd_raw"
    )
    output_dir = (
        "/home/tadas/code/deltaproteinsBristol/experimental_results/bristol_cd_plots"
    )
    ...
    # make a dictionary of "Well Position": "orientation_code" from the params_Df
    well_to_orientation_mapping = dict(
    ...
    for sample in samples:
        orientation_code = (
            well_to_orientation_mapping[sample]
            .upper()
            .replace("_", ".")
            .replace("X", "x")
            .replace("Y", "y")
        )
        print(f"Processing sample: {sample} ({orientation_code})")
        process_sample(
            sample, params_df, parsed_data, params, blank_df, orientation_code
        )
